...
面向 RG34XXSP Linux 掌机的手柄键鼠映射工具，在 X11 环境中将摇杆虚拟为键盘和鼠标输入。
...
| 键盘模式（默认模式） | 摇杆映射为虚拟键盘，按键映射为常用快捷键 |
| 鼠标模式             | 左摇杆控制鼠标移动，右摇杆控制滚轮       |

### 1. 键盘模式

#### 摇杆键盘映射

摇杆区域划分为左右两个 3×5 的网格，共 30 个虚拟按键：
...
| 第2行 | F1 F2 F3 F4 F5 | ~` 空格 -_ "' += |
| 第3行 | F6 F7 F8 F9 F10 | F11 F12 {[ }] \| |
...
| R1       | Backspace             | Delete             |
| R2       | Alt                   | Alt                |
| L3       | 锁定左摇杆当前格子    | 锁定左摇杆当前格子 |
| R3       | 锁定右摇杆当前格子    | 锁定右摇杆当前格子 |
